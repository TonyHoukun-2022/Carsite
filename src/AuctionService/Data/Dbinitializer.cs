...
{ // public => InitDb method can be accessed outside class
  // static => access modifier => InitDb belongs to the class itself rather than an instance of the class. 
  // void => return type
  // paraneter app is of type WebApplication
  public static void InitDb(WebApplication app)
  {
    // using =>  The using statement ensures that the scope is properly disposed of when it's no longer needed, 
    // createScope => Scopes are used to manage the lifetime of objects and dependencies registered with the DI container.
    using var scope = app.Services.CreateScope();

   // param =>  retrieves an instance of the AuctionDbContext class from the service provider associated with a specific scope
    SeedData(scope.ServiceProvider.GetService<AuctionDbContext>());
  }

  private static void SeedData(AuctionDbContext context)
  {
    // ensures that the database is migrated to the latest schema
    ...
    //  这行代码将拍卖品对象添加到数据库中的拍卖品表中。当调用 SaveChanges() 方法时，这些对象将被排队插入到数据库中。
